    ...
    ILogger<LlmClient> logger) : ILlmClient
{
    ...
    public string Name
    {
        get
        {
            ...
            return options.IsOllama ? $"Ollama ({options.Model})" : $"{options.Provider} ({options.Model})";
        }
    }
    ...
    public async Task<CheckResult> CheckAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if ((int)status is < 200 or >= 300)
                return CheckResult.Fail($"{options.Endpoint} answered HTTP {(int)status} when asked which models it has.");

            var available = ModelNames(payload, options.IsOllama);
            if (available.Count == 0)
                return CheckResult.Pass($"Connected to {options.Endpoint}, but it listed no models.");

            var wanted = options.Model?.Trim() ?? "";
            var present = available.Any(name =>
                string.Equals(name, wanted, StringComparison.OrdinalIgnoreCase) ||
                string.Equals(name.Split(':')[0], wanted.Split(':')[0], StringComparison.OrdinalIgnoreCase));

            if (present) return CheckResult.Pass($"Connected. '{wanted}' is available at {options.Endpoint}.");

            var pull = options.IsOllama ? $" Pull it with: ollama pull {wanted}" : "";
            return CheckResult.Fail(
                $"Connected to {options.Endpoint}, but '{wanted}' is not there. Available: {string.Join(", ", available.Take(8))}.{pull}");
        }
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        using var request = new HttpRequestMessage(method, BuildUri(options.Endpoint, path));
    ...
    private static Uri BuildUri(string? endpoint, string path)
    {
        if (!Uri.TryCreate((endpoint ?? "").Trim().TrimEnd('/') + "/", UriKind.Absolute, out var root) ||
            ...
            throw new UriFormatException($"The model endpoint '{endpoint}' is not a valid http or https URL.");

        return new Uri(root, path);
    }

    private static object OllamaBody(LlmOptions options, string system, string user) => new
    {
        model = options.Model,
        stream = false,
        format = "json",
        ...
        options = new { temperature = options.Temperature, num_predict = options.MaxOutputTokens },
    };

    private static object OpenAiBody(LlmOptions options, string system, string user) => new
    {
        model = options.Model,
