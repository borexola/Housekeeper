...
/// <summary>
/// The settings the user changed in the UI, as a JSON document shaped like the <c>Housekeeper</c> section.
...
public sealed class SettingsStore(string path, ILogger<SettingsStore> logger)
{
    ...
    public int Reset(string? section)
    {
        ...
        {
            var document = (JsonObject)Load().DeepClone();
            if (document[HousekeeperOptions.SectionName] is not JsonObject root) return 0;

            int removed;
            if (string.IsNullOrWhiteSpace(section))
            {
                removed = Count(root);
                document.Remove(HousekeeperOptions.SectionName);
            }
            else
            {
                ...
                root.Remove(section);
                if (root.Count == 0) document.Remove(HousekeeperOptions.SectionName);
            }
    ...
    private void Write(JsonObject document)
    {
        ...
        {
            // The half-written file is not the state of anything; do not leave it beside the real one.
...
public sealed class SettingsProvider : ISettingsProvider, IDisposable
{
    ...
    private volatile HousekeeperOptions _current;
    ...
    public HousekeeperOptions Current => _current;
...
public static class SettingsBinder
{
    ...
    /// </summary>
    private static readonly (string Key, Action<HousekeeperOptions, List<string>> Set)[] Lists =
    [
        ("Housekeeper:Scan:Include", (options, values) => options.Scan.Include = values),
        ("Housekeeper:Scan:Exclude", (options, values) => options.Scan.Exclude = values),
    ];

    public static HousekeeperOptions Bind(IConfiguration configuration, JsonObject? overrides)
    {
        var options = new HousekeeperOptions();
        configuration.GetSection(HousekeeperOptions.SectionName).Bind(options);
