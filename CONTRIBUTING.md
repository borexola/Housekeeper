# Contributing to HearthSense

Thanks for your interest. HearthSense is a small, focused codebase and we keep contribution friction
...
5. **Core stays pure.** `HearthSense.Core` must not reference ASP.NET, `Microsoft.Data.Sqlite`, or
   `HttpClient`. It declares the ports; `HearthSense.Api` implements them.
...
```bash
dotnet build HearthSense.slnx -c Release -warnaserror
dotnet test  HearthSense.slnx -c Release
dotnet format HearthSense.slnx --verify-no-changes   # formatting gate used by CI
```

To run it: press F5 in Visual Studio, or `dotnet run --project src/HearthSense.Api`. Either way the launch
...
This is for working on HearthSense. The ways to actually run it are the Home Assistant add-on and the
...
| | |
|---|---|
| `src/HearthSense.Core` | Pure logic and the three ports (`IHomeAssistant`, `ILlmClient`, `IStore`). |
| `src/HearthSense.Api` | Adapters and host: Home Assistant REST, LLM client, SQLite, endpoints, dashboard, scan worker. |
| `tests/HearthSense.Tests` | Unit tests over Core, store tests against real SQLite, and API tests that boot the real host with fake adapters. |
...
- **A configuration key:** add it to the options class in `Core/Options.cs`, validate it in
  `HearthSenseOptions.Validate`, then add one entry to `SettingsCatalog.Fields` in the API project. That
