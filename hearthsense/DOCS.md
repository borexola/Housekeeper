...
Describe an automation in plain English. HearthSense finds the entities you meant, has a local model draft
...
2. Install **HearthSense**, then **Start**.
...
HearthSense does not include one. Point it at whatever you already run:
...
Set this on the **Settings** page inside HearthSense, not in the add-on options. Use **Test connection**
...
Almost everything lives on HearthSense's own settings page, which persists across restarts and upgrades.
...
Open HearthSense from the sidebar and go to **Settings**. Every option is there: which Home Assistant to use,
...
HearthSense is not a life-safety system. Do not rely on it in place of smoke, CO, or leak alarms.
